...
%    usual constraints, we consider total variation (TV) constraints.
...
%    
%--------------------------------------------------------------------------
function solve_nonogram_tv(testCase)
    clc;
    close all;
    ...
    % ---------------------------------------------------------------------
    %----------------------------------------------------------------------
    [row_sequences, column_sequences, m, n, N, s_row, s_column] = read_input_file(testCase);
    
    x_true = read_solution_file(testCase, m, n, N, s_row);
    
    % Size of the augmented solution vector
    N_augmented = N + 2 * (m - 1) * n;
    ...
    % Create constraints using total variations
    ...
    A_row = [A1_row; A2_row + double(numEquations1)];
    A_col = [A1_col; A2_col];
    A_val = [A1_val; A2_val];
    ...
    numEquations    = numEquations1 + numEquations2;
    ...
    fprintf('Sparsity level: at least %d\n\n', s_row + 2*(s_column - n));
    ...
    options = optimoptions('intlinprog'         , ...
                           ...
                           'ObjectiveCutOff'    , s_row + 1);
    %}
    
    % Solve for x
    x = intlinprog([ones(N, 1); zeros(N_augmented - N, 1)], ...
    ...
    %----------------------------------------------------------------------
    path_to_save_directory = './solutions_tv/';
