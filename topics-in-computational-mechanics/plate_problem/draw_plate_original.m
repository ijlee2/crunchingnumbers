...
%--------------------------------------------------------------------------
function draw_plate_original(nodes, elements, u, gcon)
    close all;
    ...
    %----------------------------------------------------------------------
    %  Calculate the element strains and stresses
    %----------------------------------------------------------------------
    for e = 1 : numElements
        % Get the node indices
        node1Index = elements(e, 1);
        ...
        % Get the nodal displacements for FE solution
        u1x = u(gcon(node1Index, 1));
        u1y = u(gcon(node1Index, 2));
        u2x = u(gcon(node2Index, 1));
        u2y = u(gcon(node2Index, 2));
        ...
        FE_strain(e, 1) = strains(1);
        FE_strain(e, 2) = strains(2);
        FE_strain(e, 3) = strains(3)/2;
        FE_stress(e, :) = stresses;
