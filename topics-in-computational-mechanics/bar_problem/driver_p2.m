%--------------------------------------------------------------------------
%  Author: Isaac J. Lee
%  Website: https://crunchingnumbers.live
%  
%  This driver calls the assembly routine to solve the bar problem with a
%  Winkler foundation for 1, 2, 4, 8 elements with a quadratic basis.
...
function driver_p2()
    ...
    for i = 1 : size(problem_size, 1)
        ...
        assembly_1dbar_p2(problem_size{i});
        
        % Load the assembly and solution files
        load(strcat('assembly_files/assembly_1dbar_p2_numElements', problem_size{i}), 'nodes', 'elements');
        load(strcat('solution_files/solution_1dbar_p2_numElements', problem_size{i}), 'u', 'gcon');
        ...
        for e = 1 : size(elements, 1)
            ...
            plot(x3, u_h(numPoints + 1), 's', 'Color', [0 0 0], 'MarkerFaceColor', problem_color(i, :), 'MarkerSize', 6); hold on;
            ...
            plot(x3, f_h(numPoints + 1), 's', 'Color', [0 0 0], 'MarkerFaceColor', problem_color(i, :), 'MarkerSize', 6); hold on;
        end
        
        subplot(1, 2, 1);
        title('Displacement (p = 2)', 'FontSize', 30);
        ...
        subplot(1, 2, 2);
        title('Internal force (p = 2)', 'FontSize', 30);
        xlabel('x', 'FontSize', 30);
